...
class MultidomainAdversarialLoss(nn.Module):
    ...
    d_s: torch.Tensor, d_t: torch.Tensor, 
    w_s: Optional[torch.Tensor] = None, w_t: Optional[torch.Tensor] = None) -> torch.Tensor:
        f = self.grl(torch.cat((f_s, f_t), dim=0))
        d_label = self.multidomain_discriminator(f)
        d_label_s, d_label_t = d_label.chunk(2, dim=0)
        ...
        if w_s is None:
            w_s = torch.ones_like(d_s)
        if w_t is None:
            w_t = torch.ones_like(d_t)
        return 0.5 * (self.loss(d_s, d_label_s, w_s.view_as(d_s)) + self.loss(d_t, d_label_t, w_t.view_as(d_t)))
